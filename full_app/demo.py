...
#Create a tab/ page called Create Account which adds a new user to the database
    ...
    def create_closet_page(self):
        ...
        self.closet_entries = {
            ...
            'Color:': ttk.Combobox(closet_frame, values=['Black', 'White', 'Blue', 'Red']),
            'Material:': ttk.Combobox(closet_frame, values=['Cotton', 'Wool', 'Silk', 'Linen']),
    ...
    def create_outfit_generator_page(self):
        ...
        ttk.Label(generator_frame, text="Today's Dress Code:").grid(row=2, column=0, pady=5, padx=5)
    ...
    def add_clothing_item(self):
        ...
        try:
            ...
            item_name = self.closet_entries['Item Name:'].get()
            color = self.closet_entries['Color:'].get()
            material = self.closet_entries['Material:'].get()
            category = self.closet_entries['Category:'].get()
            weather_type = self.closet_entries['Weather Type:'].get()
            dress_code = self.closet_entries['Dress Code:'].get()

            # Insert item into the Clothing table
            self.cursor.execute('''
                INSERT INTO Clothing (uid, item_name, category, material, color, weather_type, dress_code)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (self.current_user, item_name, category, material, color, weather_type, dress_code))
